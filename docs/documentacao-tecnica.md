...
| `PneuAcao` (enum) | `estoque`, `conserto`, `recapagem`, `sucata`, `venda` | Cada valor carrega label, ícone/SVG e cores usados na UI. Implementa a interface `OrigemVisual` (`models/pneu_acao.dart`), que também é implementada por `EstepeOrigem` — uma origem só visual (não é localização real da API) usada para o estepe já montado no veículo (§10) |
...
- **`frota_busca_screen.dart`** — campo de placa (maiúsculas forçadas) + "Buscar" + FAB de câmera. O fluxo de câmera: `image_picker` (câmera traseira, qualidade 85) → OCR nativo → `extractPlaca` → preenche o campo, mostra toast e **dispara a busca automaticamente** (reaproveita o `_buscar`, mantendo o loading contínuo do OCR até a busca terminar). Buscar → `fetchVeiculo` → `FrotaDetalheScreen`.
- **`frota_detalhe_screen.dart`** — card do veículo (placa, frota, marca/modelo/ano/cor/tipo) + `DiagramaEixos` interativo. Toque no pneu = detalhes (bottom sheet no celular, dialog no tablet); **duplo toque no pneu** = ações de movimentação (remove o pneu do slot ao confirmar); **duplo toque em slot vazio** = montagem (insere o pneu ao confirmar). O `body` é um `Column` **sem scroll** com o diagrama num `Expanded`, então vive dentro de um `SafeArea`: sem ele o diagrama se estica até o último pixel e o eixo de baixo fica sob a barra de navegação do Android, sem nada que o revele (§12). O diagrama é atualizado localmente (otimista), sem refetch. Na montagem, o pneu vem da lista de estoque com a localização de origem, então é reinserido no slot já re-etiquetado como `FROTA` (via `Pneu.copyWith`) — sem isso o diálogo de ações trataria o pneu recém-montado como se ainda estivesse na origem e bloquearia a desmontagem até um refetch.
  - **Estepe → eixo**: o gatilho é o **duplo toque no próprio pneu do estepe já montado** (`X1`/`X2`), não num slot vazio. `showPneuAcoesDialog` (`components/pneu_acoes_dialog.dart`) detecta isso (`estepeSlotIndex(pneu.localEixo) != null`) e, se `veiculo` e `eixoSlotsVazios` (posições de eixo livres, calculadas por `eixoSlotsVazios(_eixos)` em `eixo_utils.dart`) foram informados e há pelo menos uma posição livre, acrescenta um 6º card "Eixo" à grade padrão de Estoque/Conserto/Recapagem/Sucata/Venda. Tocar nele: com uma única posição livre, vai direto pra montagem; com mais de uma, `_showEscolhaEixoDialog` pergunta qual — cada opção mostra em destaque a descrição por extenso (`descreverEixoSlot`, `eixo_utils.dart`: "Eixo 2 - Esquerdo interno", texto pensado pro motorista, que não conhece o código) e, abaixo, em menor e mais claro, o código bruto (`2EI`) como referência técnica. O restante reaproveita o mesmo `showPneuEntradaSheet` da montagem normal, mas com `origem: const EstepeOrigem()` (só visual) e `localizacaoOrigemOverride: 'FROTA'` — o pneu já está montado, só troca de posição, então o payload de `movimentarpneu` sai com `localizacaO_ORIGEM`/`localizacao` = `FROTA` em vez do label de exibição "Estepe" (que não é uma localização real da API). Ao confirmar, `_onEstepeMovidoParaEixo` (`frota_detalhe_screen.dart`) compõe as duas atualizações otimistas já existentes — `_onPneuConfirmed` (esvazia o slot de estepe) e `_onSlotVazioConfirmed` (preenche o eixo) — em vez de duplicar a lógica de slot. **Cuidado de contexto:** como esse fluxo tem um `await` (a escolha da posição) antes de abrir a montagem, ele usa o contexto de FORA do dialog (o mesmo passado a `showPneuAcoesDialog`, que sobrevive ao pop), não o `dialogContext` do próprio diálogo — que já pode estar desmontado a essa altura; mesmo cuidado que `showSlotVazioAcoesDialog`/`_navegarParaListaPneus` já tomavam pro fluxo de montagem normal. **Esta combinação de campos (`localizacaO_ORIGEM`/`localizacao` = `FROTA`, indo para uma posição de eixo do mesmo veículo) ainda não foi validada com POST real em homologação** (item 10, §15).
- **`pneu_lista_screen.dart`** — lista de todos os pneus com **filtro client-side** (nº, marca, modelo, placa, frota, série, situação, tipo, localização). Cards com badge de situação e chips. Toque: abre ações; em `selectionMode` (usado pela montagem), devolve o pneu por `Navigator.pop(context, pneu)`. **Pneu com localização `FROTA` é listado, mas tem as ações bloqueadas** (`_abrirAcoes`): o toque mostra um `showErrorToast` apontando a tela de Frotas em vez de abrir o diálogo — montagem/desmontagem exigem a posição do eixo, que só existe lá. O bloqueio fica **nesta tela, e não em `showPneuAcoesDialog`**, porque é o mesmo diálogo que o duplo toque do diagrama de eixos abre para desmontar; bloquear lá dentro mataria a desmontagem.

## 10. Diagrama de eixos

Renderização visual do chassi do veículo com um slot por posição de pneu.

- **`utils/eixo_utils.dart` → `buildEixoLayout(pneus, codEsqEixo)`**: parseia `localEixo` de cada pneu no formato `{nºeixo}{posição}` (posições `E`/`EE`/`EI`/`D`/`DE`/`DI` — esquerdo/direito, externo/interno). Se o `codEsqEixo` do veículo é um esquema conhecido, monta o **chassi inteiro** (inclusive eixos vazios, com rodado simples/duplo do esquema); pneus fora desse padrão (estepe, dado legado) ficam de fora do desenho de eixos.
- **`utils/eixo_utils.dart` → `buildEstepeLayout(pneus)`**: **estepe** é o pneu cujo `localeixo` é `X1`/`X2` (confirmado em homologação na placa FBW5J92, esquema `D`). Ele não pertence a eixo nenhum, então tem parse próprio: `buildEstepeLayout` devolve **sempre `kMaxEstepes` (2) slots** na ordem `X1`, `X2` (`null` = vazio), e `estepeSlotIndex` mapeia `X1→0`, `X2→1` (ignora `X0`/`X3` e caixa/espaços). São 2 fixos porque o esquema de eixos não diz quantos estepes o veículo comporta, e 2 é o teto da operação. O `DiagramaEixos` desenha esses slots numa **faixa horizontal acima do chassi** (`EstepeBand`, irmã do frame — vale para qualquer esquema, moto inclusive), com os mesmos `PneuTile`s dos eixos: tocar vê detalhes, duplo toque movimenta/monta. Por ser faixa (e não coluna lateral), **não disputa largura** com os eixos — os dois pneus somam ~60pt, folgados até no celular pequeno; o custo é **altura**, descontada do teto do frame no cálculo (`estepeBandHeight`) para não gerar scroll à toa.
...
- **`primitives.dart`**: os desenhos em si, com `CustomPainter` (API do Flutter para desenhar direto no canvas): `TirePainter` (pneu visto de cima com sulcos e sipes), `EmptyTirePainter` (slot vazio com borda tracejada), parachoque, pino-rei, hubs e labels `E1/E2...`. Dimensões dobram no tablet. O `PneuTile` (único lugar que desenha o nº do pneu, inclusive na moto) põe o número num **`FittedBox(scaleDown)`**: a caixa do rótulo tem a largura do pneu (28pt no celular) e o número vem da API com **até 5 dígitos**, que nessa fonte chegam a ~31pt — sem o FittedBox o `Text` corta o excedente **em silêncio** (`TextOverflow.clip` é o default do Flutter, não gera erro, e por isso a matriz responsiva não pega: conteúdo cortado não estoura nada). No rodado duplo não há nem pra onde transbordar — os pneus ficam a 9pt um do outro. `scaleDown` só reduz quando não cabe, então 3–4 dígitos seguem no tamanho de design.
...
- **Barra de navegação do Android (*edge-to-edge*)**: o app declara `targetSdk = 36` (default do Flutter), e a partir do 35 o Android **obriga** o modo edge-to-edge — não há mais opt-out. Isso significa que o sistema entrega a tela INTEIRA e desenha a barra de navegação (3 botões, ~48dp, ou pílula de gesto, ~24dp) **por cima** do app: recuar o conteúdo é responsabilidade nossa, lendo `MediaQuery.padding`.
  - **Convenção**: **toda tela envolve o `body` num `SafeArea`** — as com AppBar usam `SafeArea` puro (o Scaffold já zera o inset do topo quando há AppBar, então na prática ele vale pelo rodapé e pelas laterais, onde a barra fica em paisagem). Onde o fundo é gradiente (`login_screen`), o `SafeArea` fica **por dentro**, envolvendo só o conteúdo, para o gradiente seguir sangrando até a borda. `splash_screen` é a exceção deliberada: fundo gradiente com logo centralizado, sem nada no rodapé para recuar.
  - É **crítico em tela sem scroll**, onde o que fica sob a barra é inalcançável: foi o caso do `frota_detalhe_screen` (diagrama num `Expanded`, §9) e da barra Cancelar/Confirmar dos bottom sheets (`folgaInferiorBotoes`, em `components/shared/form_helpers.dart`). Nas telas que rolam o `SafeArea` é preventivo — o conteúdo já era alcançável por scroll.
  - **Não confundir `padding` com `viewInsets`**: `viewInsets` é o **teclado**, `padding` é a **barra de navegação**; são campos distintos do MediaQuery e tratar um não cobre o outro. As duas coisas se compõem sozinhas: com o teclado aberto, `padding.bottom` zera (o teclado já cobre a barra) e quem sobe o sheet é o `Padding(viewInsets.bottom)`.
...
- **Matriz responsiva** (`test/screens/responsive_matrix_test.dart` + `test/helpers/test_viewport.dart`): **6 telas × 10 perfis de dispositivo = 60 testes**. Perfis: celulares pequeno/padrão/grande, celular com barra de navegação, fonte grande (escala 1.3), paisagem, tablet retrato/paisagem/fonte grande. O teste renderiza cada tela em cada viewport e conta com o detector implícito de overflow do Flutter (RenderFlex overflow → erro → teste falha). `usePhoneViewport` existe porque o viewport default do `flutter test` (800×600) cruzaria o breakpoint de tablet. O perfil com barra de navegação (`DeviceProfile.barraNavegacao` → `view.padding`) cobre o overflow que o recuo do rodapé pode causar ao encurtar o espaço útil; que o conteúdo não fique **sob** a barra é asserção explícita dos testes por tela — a matriz não pega isso sozinha, porque conteúdo encoberto não estoura nada.
- **Barra de botões, teclado e barra de navegação** (`test/components/bottom_sheet_teclado_test.dart`): nos três bottom sheets de movimentação, a barra fixa Cancelar/Confirmar é testada contra os dois insets do sistema. **Teclado**: inset de 340pt num celular estreito — a barra não pode estourar o layout e tem de ficar acima da linha do teclado; testa também que arrastar o formulário pra baixo (com `BouncingScrollPhysics`) **dispensa o teclado**. **Barra de navegação**: 48pt de `view.padding` num celular **baixo** (568) e na ação de formulário mais longo (sucata) — as duas condições são necessárias, porque o sheet tem altura mínima de design e, quando o formulário é menor que ela, a sobra cai embaixo dos botões e afasta-os da barra por acidente, escondendo a regressão. As asserções medem o **botão**, não o texto: o rótulo é centralizado em 56pt de altura e carrega ~20pt de folga própria, que mascara o problema (o texto escapa enquanto a área de toque segue encoberta).
...
- **Toasts em widget test** (exemplo em `test/screens/pneu_lista_screen_test.dart`): o `fluttertoast` é nativo (`MethodChannel` `PonnamKarthik/fluttertoast`) e não existe no ambiente de teste — como `showErrorToast` não espera o retorno, o `MissingPluginException` vira falha assíncrona solta e derruba o teste. Todo teste que exercita um toast precisa (1) trocar o canal por um mock (`setMockMethodCallHandler`, que de quebra deixa asseverar a mensagem em `arguments['msg']`) e (2) **adiantar o relógio** além da duração do toast: o pacote agenda um `Future.delayed` interno e, pendente, ele faz o binding falhar com "A Timer is still pending".
...
10. **Movimentação "Estepe → eixo"** (mover o estepe já montado para uma posição de eixo do mesmo veículo, §9/§10) monta o payload de `movimentarpneu` com `localizacaO_ORIGEM`/`localizacao` = `FROTA` e `localeixo` = destino, mesma forma de uma montagem normal — mas essa combinação específica de campos ainda não foi testada com um POST real em homologação.
